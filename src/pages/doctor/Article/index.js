...
function DoctorArticlePage() {
	...
	useEffect(() => {
		const getUser = async () => {
			try {
				...
				&name_category=${search.name_category}&is_show=${search.is_show}`
				const response = await http.get('article/doctor' + queryParams);
   ...
   return (
		<>
			<ToastContainer />
			<TitleAdmin>Bài viết bác sĩ</TitleAdmin>
